...
# One-time interactive login to bootstrap a persisted session for a
# Playwright-based fetcher (see scripts/<portal>_login_bootstrap.py).
# Dedicated minimal venv -- scanner/requirements.txt, not the full `venv`
# recipe's three-services deps. Needs the whole scanner requirements (not
# just playwright) because these bootstrap scripts import from their
# fetcher module, which pulls in scanner.fetchers' package init (imports
# every portal module, e.g. httpx via base.py) as a side effect.
# Shared across portals -- identical deps, no reason for a venv each.
...
_session_bootstrap script:
    #!/usr/bin/env bash
    set -euo pipefail
    venv_dir="${TMPDIR:-/tmp}/jobscout-session-bootstrap-venv"
...
linkedin-session-bootstrap:
    just _session_bootstrap scripts/linkedin_login_bootstrap.py

xing-session-bootstrap:
    just _session_bootstrap scripts/xing_login_bootstrap.py
